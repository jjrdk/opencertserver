...
public sealed class McpServerCertificateOperationsSteps
{
    ...
    {
        // Issue cert via shared state so assertion steps can read it
        await TestSharedState.IssueAsync(_fixture.McpServer, "test-ops-1");
    }
    ...
    public async Task WhenListForInventory()
    {
        var result = await _fixture.InvokeMcpToolAsync("list_certificates", new { page = 0, pageSize = 500 });
    ...
    [Then("the returned certificate must have a Pem field containing \\\"-----BEGIN CERTIFICATE-----\\\"")]
    public void ThenMustHavePem()
    {
        Assert.NotNull(TestSharedState.SignedCert);
        Assert.NotNull(TestSharedState.SignedCert.Pem);
        Assert.Contains("-----BEGIN CERTIFICATE-----", TestSharedState.SignedCert.Pem);
    }

    [Then("the returned certificate must have a PemChain field containing \\\"-----BEGIN CERTIFICATE-----\\\"")]
    public void ThenMustHavePemChain()
    {
        Assert.NotNull(TestSharedState.SignedCert);
        Assert.NotNull(TestSharedState.SignedCert.PemChain);
        Assert.Contains("-----BEGIN CERTIFICATE-----", TestSharedState.SignedCert.PemChain);
    }

    [Then("the PemChain must contain at least 2 certificates (intermediate + root)")]
    public void ThenPemChainMustHaveMultipleCerts()
    {
        Assert.NotNull(TestSharedState.SignedCert);
        var pemChain = TestSharedState.SignedCert.PemChain ?? string.Empty;
        var count = pemChain.Split("-----BEGIN CERTIFICATE-----").Length - 1;
        Assert.True(count >= 2, $"PemChain contains {count} certificates, expected at least 2");
    }
